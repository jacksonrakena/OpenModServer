...
namespace OpenModServer.Areas.Account.Pages
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        private readonly IUserStore<OmsUser> _userStore;
        private readonly IUserEmailStore<OmsUser> _emailStore;
        ...
        private OmsUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<OmsUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(OmsUser)}'. " +
                    $"Ensure that '{nameof(OmsUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the external login page in /Areas/Identity/Pages/ExternalLogin.cshtml");
            }
        }

        private IUserEmailStore<OmsUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<OmsUser>)_userStore;
        }
    }
}
